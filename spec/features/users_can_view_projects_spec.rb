...
RSpec.feature "Users can view a list of projects" do
  ...
  let!(:unassigned_project) {
    create(
      :project,
      urn: 100001,
      provisional_completion_date: Date.today.beginning_of_month + 3.years
    )
  }
  let!(:user_1_project) {
    create(
      ...
      urn: 100002,
      caseworker: user_1,
      provisional_completion_date: Date.today.beginning_of_month + 2.year
    )
  }
  let!(:user_2_completed_project) {
    create(
      ...
      provisional_completion_date: Date.today.beginning_of_month + 6.months,
  ...
  let!(:user_2_project) {
    create(
      ...
      caseworker: user_2,
      regional_delivery_officer: regional_delivery_officer,
      provisional_completion_date: Date.today.beginning_of_month + 1.years
    )
  }
  ...
  private def page_has_project(project)
    ...
    within urn.ancestor("li") do
      ...
      expect(page).to have_content("Provisional conversion date: #{project.provisional_completion_date.to_formatted_s(:govuk)}")
