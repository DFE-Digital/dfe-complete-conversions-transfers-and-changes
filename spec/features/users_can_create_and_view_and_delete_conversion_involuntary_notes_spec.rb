...
RSpec.feature "Users can create and view and delete conversion involuntary notes" do
  ...
  scenario "User creates and views task level notes" do
    visit conversions_involuntary_project_edit_task_path(project_id, task_identifier)
    ...
    expect(page).to have_link("Cancel", href: conversions_involuntary_project_edit_task_path(project, task_identifier))

    fill_in "Enter note", with: new_note_body

    click_button I18n.t("note.new.save_note_button")

    expect(page).to have_current_path(conversions_involuntary_project_edit_task_path(project_id, task_identifier))
  ...
  scenario "User edits a task level note" do
    ...
    expect(page).to have_link("Back", href: conversions_involuntary_project_edit_task_path(project_id, task_identifier))
