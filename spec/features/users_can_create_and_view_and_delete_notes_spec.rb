...
RSpec.feature "Users can create and view notes" do
  ...
  before do
    ...
    travel_to Date.yesterday do
      create(:note, user: user, conversion_project: project)
    end
  ...
  scenario "User creates and views notes" do
    visit conversion_project_notes_path(project_id)
    ...
    expect(page).to have_current_path(new_conversion_project_note_path(project))
    expect(page).to have_link("Back", href: conversion_project_notes_path(project_id))

    fill_in "Enter note", with: new_note_body

    click_button("Add note")

    expect(page).to have_current_path(conversion_project_notes_path(project_id))
