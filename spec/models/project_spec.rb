...
RSpec.describe Project, type: :model do
  describe "Columns" do
    ...
    it { is_expected.to have_db_column(:provisional_completion_date).of_type :date }
  ...
  describe "Validations" do
    ...
    describe "#provisional_completion_date" do
      it { is_expected.to validate_presence_of(:provisional_completion_date) }

      context "when the date is not on the first of the month" do
        subject { build(:project, provisional_completion_date: Date.today.months_since(6).at_end_of_month) }

        it "is invalid" do
          expect(subject).to_not be_valid
          expect(subject.errors[:provisional_completion_date]).to include(I18n.t("activerecord.errors.models.project.attributes.provisional_completion_date.must_be_first_of_the_month"))
        end
      end

      context "when date is today" do
        subject { build(:project, provisional_completion_date: Date.today) }

        it "is invalid" do
          expect(subject).to_not be_valid
          expect(subject.errors[:provisional_completion_date]).to include(I18n.t("activerecord.errors.models.project.attributes.provisional_completion_date.must_be_in_the_future"))
        end
      end

      context "when date is in the past" do
        subject { build(:project, provisional_completion_date: Date.yesterday) }
  ...
  describe "by_target_completion_date scope" do
    ...
    it "shows the project that complete earliest first" do
      last_project = create(:project, provisional_completion_date: Date.today.beginning_of_month + 3.years)
