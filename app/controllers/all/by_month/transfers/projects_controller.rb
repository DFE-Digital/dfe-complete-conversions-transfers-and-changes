class All::ByMonth::Transfers::ProjectsController < ApplicationController
  ...
  private def get_projects_for_date_range
    @pager, @projects = pagy_array(
      ByMonthProjectFetcherService.new.transfer_projects_by_date_range(@from_date, @to_date)
    )
  end

  private def redirect_if_dates_incorrect
    redirect_to date_range_this_month_all_by_month_transfers_projects_path,
