class ServiceSupport::Upload::Gias::UploadEstablishmentsForm
  ...
  def save
    ...
    Import::GiasImportJob.set(wait_until: Date.tomorrow.in_time_zone.change(hour: IMPORT_TIME)).perform_later(file_path_with_timestamp.to_s, @user)
  end
  ...
  private def file_headers
    ...
    unless Import::Gias::EstablishmentCsvImporterService.new(@uploaded_file.path).required_column_headers_present?
