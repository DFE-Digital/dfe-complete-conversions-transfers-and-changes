...
- The team a user is associated with is now shown on the All projects, by team
  view.
- User accounts can now be disabled which prevents the user from accessing the
  application or from showing up in user selections, disabled accounts can be
  enabled.
...
- The users view is now split into enabled and disabled user accounts.
